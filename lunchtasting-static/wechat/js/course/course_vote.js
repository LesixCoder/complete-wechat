            ...
            return {
                voteFn: function (me) {
                    ...
                    jQuery.post("/course/vote/add", data, function (msg) {
                        ...
                        var err = msg['message'];
                        if (code == 100) {
                            var parent = jQuery('.voteBtn[js-uid=' + _this.vote_uid + ']').parents('.figure-l-fig');
                            var count = parseInt(parent.find('.vote-count').html());
                            count += 1;
                            parent.find('.vote-count').html(count);
                            //jQuery('.voteBtn[js-uid=' + _this.vote_uid + ']').attr('js-status', '1');
                            jQuery('.voteBtn').attr('js-status', '1');
                            jQuery('.voteBtn').addClass('vote-end').html('<i>已投</i>');
                            _this.voteBtn.trigger("render");
                            var imgBox = jQuery('.voteBtn[js-uid=' + _this.vote_uid + ']').parents('article').find('.figure-r-span');
                            imgBox.prepend('<a href="javascript:;"><img src="'+msg.data.user_img_url+'" alt=""></a>');
