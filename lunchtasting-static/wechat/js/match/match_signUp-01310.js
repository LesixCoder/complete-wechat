...
jQuery(function() {
    ...
    jQuery("#signBtn").click(function() {
        var reg1 = /^[\u4e00-\u9fa5]{1,5}$|^[\dA-Za-z_]{1,10}$/; //匹配昵称
        var reg2 = /^(0|86|17951)?(13[0-9]|15[012356789]|17[678]|18[0-9]|14[57])[0-9]{8}$/; //手机号
        var reg3 = /^[\u2E80-\u9FFF]+$/; //姓名
        var reg4 = /^(\d{6})(\d{4})(\d{2})(\d{2})(\d{3})([0-9]|X)$/; //身份证
        var timeStr = jQuery("#user-age").val();
        // if(timeStr == ''){
        //     return alert('请输入出生日期');
        // }

        var data = {
            name: jQuery("#nickname").val(), //昵称
            sex: jQuery(".wide option:selected").val(), //性别
            birth: jQuery("#user-age").val(), //出生日期
            phone: jQuery("#tel").val(), //手机号
            code: jQuery("#verCode").val(), //验证码
            real_name: jQuery("#realName").val(), //真实姓名
            certificate: jQuery("#identity").val(), //身份证
            invite_code: jQuery("#invite_code").val(), //邀请码
            match_id: jQuery("#matchId").val()
        };

        if (!reg1.test(data.name)) {
            return alert("请输入正确的昵称！");
        } else if (timeStr == '') {
            return alert('请输入出生日期');
        } else if (!reg2.test(data.phone)) {
            return alert("请输入正确的手机号！");
        } else if (!reg3.test(data.real_name)) {
            return alert("请输入正确的姓名！");
        } else if (!reg4.test(data.certificate)) {
            return alert("请输入正确的身份证！");
        }

        if (data.name !== '' && data.sex !== '' && data.birth !== '' && data.phone !== '' && data.code !== '' && data.real_name !== '' && data.certificate !== '') {
            if (confirm("确定要提交信息吗?")) {
                jQuery.ajax({
                    url: '/match/signup/do',
                    data: data,
