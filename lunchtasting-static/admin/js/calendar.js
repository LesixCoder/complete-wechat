...
(function($) {
    ...
    $.fn.simpleDatepicker = function(options) {
		...
		function newDatepickerHTML () {
			...
			var table = jQuery('<table class="datepicker" cellpadding="0" cellspacing="0" id="riqiDate"></table>');
			...
			table.append('<tbody></tbody>');
				...
				for (var i=0;i<12;i++) monthselect += '<option value="'+i+'">'+months[i]+'</option>';
				monthselect += '</select>';
			
				// year select field
				var yearselect = '<select name="year">';
				for (var i=0;i<=opts.endyear - opts.startyear;i++) yearselect += '<option>'+years[i]+'</option>';
				yearselect += '</select>';
			...
			if ( jQuery(this).is('input') && 'text' == jQuery(this).attr('type')) {
				...
				jQuery(this).click(function (ev) {
					...
					if (false == jQuery.data($this.get(0), "simpleDatepicker").hasDatepicker) {
						...
						jQuery("select", datepicker).bind('change', function (ev) { loadMonth (null, $this, datepicker, chosendate); ev.stopPropagation();});
