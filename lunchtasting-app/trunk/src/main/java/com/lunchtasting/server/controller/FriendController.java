...
public class FriendController {
	...
	@Autowired
	private MessageNotificationBIZ notificationBIZ;
	...
	public Object add(HttpServletRequest request) throws Exception {
		...
		try {
			...
			if(result){
				 ...
				 * 推送
				 */
				boolean checkSame = notificationBIZ.checkSameFriend(desUserId,userId);
				if(checkSame){
					notificationBIZ.pushMessage(desUserId,userId, null
							, MessageNotificatioType.FRIEND_FOLLOW.getType(), null, null, null);
				}
	...
	public Object followList(HttpServletRequest request) throws Exception {
		...
		try {
			...
			dataMap.put("list", friendBIZ.queryFollowList(userId,desUserId,isLoginUser,(pPage-1)*VariableConfig.PAGE_SIZE, VariableConfig.PAGE_SIZE));
	...
	public Object fansList(HttpServletRequest request) throws Exception {
		...
		try {
			 ...
			 */
			Map dataMap = new HashMap();
			dataMap.put("list", friendBIZ.queryFansList(userId,desUserId,isLoginUser,(pPage-1)*VariableConfig.PAGE_SIZE, VariableConfig.PAGE_SIZE));
			 ...
			 */
			if(pPage == 1){
				notificationBIZ.updateIsRead(userId, MessageNotificatioType.FRIEND_FOLLOW.getType());
			}
