...
public class UserController {
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/center_v2_2_1" , method=RequestMethod.POST)
	@ResponseBody
	public Object centerV2_2_1(HttpServletRequest request) throws Exception {
		...
		try {
			 ...
			 * 未读消息总数
			 */
			Map unreadNotifyMap = notificationBIZ.getUnreadMap(userId);
			
			Map dataMap = new HashMap();
			userMap.put("note_count", noteCount+likeCount);
			dataMap.put("user", userMap);
			dataMap.put("unread_notify", unreadNotifyMap);
	...
	@RequestMapping(value = "/refresh/message" , method=RequestMethod.POST)
	@ResponseBody
	public Object refreshMessage(HttpServletRequest request) throws Exception {
		
		try {
			long userId = EncryptAuth.getUserId(request);
			Map dataMap = new HashMap();
			dataMap.put("unread_notify", notificationBIZ.getUnreadMap(userId));
			return MapResult.build(MapResult.CODE_SUCCESS, MapResult.MESSAGE_SUCCESS, dataMap, request);
		} catch (Exception e) {
			e.printStackTrace();
			return MapResult.build(MapResult.CODE_SYS_ERROR, MapResult.MESSAGE_ERROR, request);
