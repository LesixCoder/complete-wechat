	...
	public Object createLike(HttpServletRequest request)  {
		...
		try {
			...
			if(result){
				 ...
				 * 推送消息
				 */
				boolean checkSame = notificationBIZ.checkSameNoteLike(note.getUserId(),userId, noteId);
				if(checkSame){
					notificationBIZ.pushMessage(note.getUserId(),userId, noteId,
							MessageNotificatioType.NOTE_LIKE.getType(), null, null, note.getImgUrl());
				}
