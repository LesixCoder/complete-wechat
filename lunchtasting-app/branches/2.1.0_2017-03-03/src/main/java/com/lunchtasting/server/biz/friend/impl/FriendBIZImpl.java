...
public class FriendBIZImpl implements FriendBIZ {
	...
	@Override
	public List queryFollowList(Long userId,Long desUserId,Long loginUserId,Integer page, Integer pageSize) {
		List list = friendDAO.queryFollowList(userId,desUserId, page, pageSize);
		if(ValidatorHelper.isNotEmpty(list)){
			...
			for(int i =0 ;i<list.size();i++){
				 ...
				 */
				if(loginUserId != null 
						&& loginUserId.longValue() == desUserId.longValue()){
					map.put("is_follow", 1);
				}else{
	...
	public List queryFansList(Long userId,Long desUserId, Integer page, Integer pageSize) {
		List list = friendDAO.queryFansList(userId,desUserId, page, pageSize);
		if(ValidatorHelper.isNotEmpty(list)){
			List newList = new ArrayList();
			for(int i =0 ;i<list.size();i++){
				HashMap map = (HashMap) list.get(i);
