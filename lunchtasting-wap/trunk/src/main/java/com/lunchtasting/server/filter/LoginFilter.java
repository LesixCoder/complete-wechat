package com.lunchtasting.server.filter;
...
public class LoginFilter implements Filter {
	...
	public void doFilter(ServletRequest req, ServletResponse res,
		...
		User user = new User();
		user = (User)session.getAttribute(VariableHelper.LOGIN_SESSION_USER);
		/**
		...
		if(url.equalsIgnoreCase("/")){
			if(user != null){
				...
				response.sendRedirect(CommonHelper.gotoWebRoot(request)+"index");
				return;
		...
		if(!loginList.contains(url)){
			if(user == null){
				 ...
				 * 跳转到登陆页
				 */
				response.sendRedirect(CommonHelper.gotoWebLogin(request));
				return;
	...
	protected void initCaptchaService(final FilterConfig config) {
		...
		userBIZ = (UserBIZ) context.getBean("userBIZ");
	}
	...
	protected void loginNotFilter(){
		...
		loginList.add("/wechat/wechatInvoking");
	}
			
	/**
	 * 做一个假cookie 来欺骗encodeRedirectURL() 取消url后面的jsessionid
	 * @param request,response
	 */
	protected void doFackCookie(HttpServletRequest request,HttpServletResponse response){
