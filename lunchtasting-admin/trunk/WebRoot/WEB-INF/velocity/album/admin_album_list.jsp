...
<!-- 					    <a href="go" class="btn btn-secondary btn-sm btn-icon icon-left" style="color:#ffffff;">查询</a> -->
					    <a href="goAlbumSave" class="btn btn-info btn-sm btn-icon icon-left" style="color:#ffffff;">添加</a>
					</h3>
				...
				<script type="text/javascript">
					...
					jQuery(document).ready(function($){
						    albumList = jQuery("#example-2").dataTable({
							sAjaxSource: "queryAlbumList",
							bServerSide: true,
							...
							aoColumns: [
								...
								{"mDataProp": "bizType","sClass": "thead-tr-th"},
							...
							columnDefs: [
							{
								...
								"render": function(data, type, row,full) { //返回自定义内容
									var html = "";
									html += "<a href='goAlbumSave?id="+data+"' class='btn btn-secondary btn-sm btn-icon icon-left'>修改</a>";
									html += "<a href='javascript:void(0);' onclick='deleteAlbum(this);' id="+data+" class='btn btn-danger btn-sm btn-icon icon-left'>删除</a>";
									if(row.flag == 0){
							...
							if(confirm("确定要删除?")){
								jQuery.ajax({
								    ...
								    if(data.flag = "success"){
								        alert("删除成功!");
								        albumList.fnPageChange(parseInt($(".pagination").find("li[class='paginate_button active'] a").html())-1);
								        albumList.fnReloadAjax();
								    }else{
								        alert("删除失败!");
								       }
								    },
					...
					<table class="table table-bordered table-striped " id="example-2" data-pattern="priority-columns">
						<thead>
							<tr>
								...
								<th style="font-size: 13px;">相册类型</th>
								<th style="font-size: 13px;">相册名称</th>
							</tr>
						</thead>
