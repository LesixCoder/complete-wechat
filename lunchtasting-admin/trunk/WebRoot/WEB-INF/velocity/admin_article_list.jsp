...
<!-- 					</div> -->
		...
		<div class="main-content">
			...
			<div class="panel panel-default">
				<form method="post" class="form-horizontal" id="agents-form">
					<div class="form-group" align="center">
						<label class="col-sm-2 control-label"
							for="restaurant.restaurantName">文章名称</label>
						<div class="col-sm-2">
				...
				<div class="panel-heading" style="padding-left: 17px;">
					<h3 class="panel-title">
					    <a href="javascript:selectArticle();" class="btn btn-secondary btn-sm btn-icon icon-left" style="color:#ffffff;">查询</a>
						<a href="goArticleSave" class="btn btn-info btn-sm btn-icon icon-left" style="color:#ffffff;">添加</a>
					</h3>
				...
				<script type="text/javascript">
					...
					jQuery(document).ready(function($){
						    articleList = jQuery("#example-2").dataTable({
							sAjaxSource: "queryArticleList",
							bServerSide: true,
							...
							"fnServerParams" : function (aoData) {
								...
								{ "name": "typeSelect", "value": jQuery("#typeSelect").val() }
								    );
							},
							aoColumns: [
								...
								{"mDataProp": "tag","sClass": "thead-tr-th"}
							],
							columnDefs: [{
								"targets": [0],
								"data": "newId", //数据列名
								"render": function(data, type, row,full) { //返回自定义内容
									var html = "";
									html += "<a href='goArticleSave?id="+data+"' class='btn btn-secondary btn-sm btn-icon icon-left'>修改</a>";
									html += "<a href='javascript:void(0);' onclick='deleteArticle(this);' id="+data+" class='btn btn-danger btn-sm btn-icon icon-left'>删除</a>";
									if(row.flag == 0){
									...
									html += "<a href='javascript:void(0);' onclick='preview(\""+row.name+"\",\""+row.imgUrl+"\",this);' id="+data+" class='btn btn-orange btn-sm btn-icon icon-left'>预览</a>";
									html += "<a href='javascript:void(0);' onclick='topArticle(this);' id="+data+" class='btn btn-secondary btn-sm btn-icon icon-left'>置顶</a>";
								return html;
								}
							}
							],
							...
							if(confirm("确定要删除?")){
								jQuery.ajax({
								    ...
								    if(data.flag = "success"){
								        alert("删除成功!");
								        articleList.fnPageChange(parseInt($(".pagination").find("li[class='paginate_button active'] a").html())-1);
								        articleList.fnReloadAjax();
								    }else{
								        alert("删除失败!");
								       }
								    },
						...
						//预览
						function preview(name,imgUrl,e){
					...
					<table class="table table-bordered table-striped " id="example-2" data-pattern="priority-columns">
						<thead>
							<tr>
								...
								<th style="font-size: 13px;">文章名称</th>
								<th style="font-size: 13px;">文章简介</th>
								<th style="font-size: 13px;">标签</th>
							</tr>
						</thead>
	...
	<div class="modal fade" id="modal-1">
		<div class="modal-dialog">
            ...
            <div class="body-details">
                <!--图文混编所在位置-->
                <h3 style="text-align: center;margin: 4% 0 1%;color: #000;font-size: .22rem;" id="articleName"></h3>
                <h5 style="text-align: right;color: #20b3a4;"></h5>
                <div id="imgText">
                </div>
            </div>
        </section>
