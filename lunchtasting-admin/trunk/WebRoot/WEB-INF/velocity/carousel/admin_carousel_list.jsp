...
<!-- 					    <a href="javascript:selectCourse();" class="btn btn-secondary btn-sm btn-icon icon-left" style="color:#ffffff;">查询</a> -->
					    <a href="goCarouselSave" class="btn btn-info btn-sm btn-icon icon-left" style="color:#ffffff;">添加</a>
					</h3>
				...
				<script type="text/javascript">
					...
					jQuery(document).ready(function($){
						    carouselList = jQuery("#example-2").dataTable({
							sAjaxSource: "queryCarouselList",
							bServerSide: true,
					...
					/* 		"fnServerParams" : function (aoData) {
							...
							aoColumns: [
								...
								{"mDataProp": "depict","sClass": "thead-tr-th"},
								{"mDataProp": "url", "sClass": "thead-tr-th"},
							],
							columnDefs: [
							{
								"targets": [0],
								...
								"render": function(data, type, row,full) { //返回自定义内容
									var html = "";
									html += "<a href='goCarouselSave?id="+data+"' class='btn btn-secondary btn-sm btn-icon icon-left'>修改</a>";
									html += "<a href='javascript:void(0);' onclick='deleteCarousel(this);' id="+data+" class='btn btn-danger btn-sm btn-icon icon-left'>删除</a>";
									if(row.flag == 0){
							...
							if(confirm("确定要启用?")){
								jQuery.ajax({
								    ...
								    if(data.flag = "success"){
								        alert("启用成功!");
								        carouselList.fnPageChange(parseInt($(".pagination").find("li[class='paginate_button active'] a").html())-1);
								        carouselList.fnReloadAjax();
								    }else{
								        alert("启用失败!");
								       }
								    },
					...
					<table class="table table-bordered table-striped " id="example-2" data-pattern="priority-columns">
						<thead>
							<tr>
								...
								<th style="font-size: 13px;">描述</th>
								<th style="font-size: 13px;">链接</th>
							</tr>
						</thead>
