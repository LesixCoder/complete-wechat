...
<!-- 					    <a href="javascript:selectCourse();" class="btn btn-secondary btn-sm btn-icon icon-left" style="color:#ffffff;">查询</a> -->
					    <a href="goTopicSave" class="btn btn-info btn-sm btn-icon icon-left" style="color:#ffffff;">添加</a>
					</h3>
				...
				<script type="text/javascript">
					...
					jQuery(document).ready(function($){
						    topicList = jQuery("#example-2").dataTable({
							sAjaxSource: "queryTopicList",
							bServerSide: true,
							...
							columnDefs: [
							{
								...
								"render": function(data, type, row,full) { //返回自定义内容
									var html = "";
									html += "<a href='goTopicSave?id="+data+"' class='btn btn-secondary btn-sm btn-icon icon-left'>修改</a>";
									html += "<a href='javascript:void(0);' onclick='deleteTopic(this);' id="+data+" class='btn btn-danger btn-sm btn-icon icon-left'>删除</a>";
									if(row.flag == 0){
							...
							if(confirm("确定要删除?")){
								jQuery.ajax({
								    ...
								    if(data.flag = "success"){
								        alert("删除成功!");
								        topicList.fnPageChange(parseInt($(".pagination").find("li[class='paginate_button active'] a").html())-1);
								        topicList.fnReloadAjax();
								    }else{
								        alert("删除失败!");
								       }
								    },
