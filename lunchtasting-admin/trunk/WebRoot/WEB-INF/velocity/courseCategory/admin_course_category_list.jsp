				...
				<form class="form-horizontal">
					<div id="hideDiv">
						<div class="form-group">
							...
							<div class="col-sm-2" lang="div1">
								<select class="form-control selectpicker bla bla bli" data-live-search="true" id="level"
...
<!-- 									<option value="2">二级</option> -->
				...
				<div class="panel-heading" style="padding-left: 17px;">
					<h3 class="panel-title">
					    ...
					    <a href="goCourseCategorySave" class="btn btn-info btn-sm btn-icon icon-left" style="color:#ffffff;">添加</a>
					</h3>
				...
				<script type="text/javascript">
					...
					jQuery(document).ready(function($){
						    courseCategoryList = jQuery("#example-2").dataTable({
							sAjaxSource: "queryCourseCategoryList",
							bServerSide: true,
							...
							aoColumns: [
								...
								{"mDataProp": "level","sClass": "thead-tr-th"}
							],
							columnDefs: [{
								"targets": [0],
								"data": "id", //数据列名
								"render": function(data, type, row,full) { //返回自定义内容
									var html = "";
									html += "<a href='goCourseCategorySave?id="+data+"' class='btn btn-secondary btn-sm btn-icon icon-left'>修改</a>";
									html += "<a href='javascript:void(0);' onclick='deleteCourseCategory(this,\""+row.level+"\");' id="+data+" class='btn btn-danger btn-sm btn-icon icon-left'>删除</a>";
									if(row.flag == 0){
							...
							},
							{
								...
								"render": function(data,type,row, full) {
									if(row.level == 1){
										return "<span style='color:#50A625;'>"+data+"</span>";
									}else if(row.level == 2){
										return "<span style='color:#38044B;'>"+data+"</span>";
									}
								}
							},
							{
								...
								"render": function(data, type, full) { 
									if(data == 1){
										return "<span style='color:#50A625;'>一级</span>";
									}else if(data == 2){
										return "<span style='color:#38044B;'>二级</span>";
									}
								}
							}
							],
						    ...
						    //删除
						function deleteCourseCategory(e,level){
						    var id = e.id;
						    var data = {
							    id  : id,
							    type:"del",
						    ...
						    if(level == 1){
						    if(confirm("您要删除的是一级类别,删除后其下的二级类别将一并删除,确定要删除?")){
								jQuery.ajax({
								    ...
								    if(data.flag = "success"){
								        alert("删除成功!");
								        courseCategoryList.fnPageChange(parseInt($(".pagination").find("li[class='paginate_button active'] a").html())-1);
								        courseCategoryList.fnReloadAjax();
								    }else{
								        alert("删除失败!");
								       }
								    },
					...
					<table class="table table-bordered table-striped " id="example-2" data-pattern="priority-columns">
						<thead>
							<tr>
								...
								<th style="font-size: 13px;">分类名称</th>
								<th style="font-size: 13px;">级别</th>
							</tr>
						</thead>
