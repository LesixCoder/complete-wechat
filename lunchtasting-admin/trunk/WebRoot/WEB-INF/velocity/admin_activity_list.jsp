							...
							<div class="col-sm-2" style="color: #cc3f44;">
								<div class="input-group">
									<input type="text" class="form-control daterange"
										id="activityDate" name="activityDate" readonly="readonly"
					...
					jQuery(document).ready(function($){
							...
							"fnRowCallback": function (nRow, aoData, iDisplayIndex) {
								if(aoData.content != ""){
								   if (aoData.content.length > 10) {
					                jQuery('td:eq(2)', nRow).html( aoData.content.substr(0, 10)+"...");
					                }
								}
					            if(aoData.name != ""){
					            ...
					            if(aoData.address != ""){
					              if (aoData.address.length > 10) {
					                jQuery('td:eq(4)', nRow).html( aoData.address.substr(0, 10)+"...");
					                }
					            }
					        },
							...
							columnDefs: [{
								...
								"render": function(data, type, row,full) { //返回自定义内容
									var html = "";
									html += "<a href='goActivitySave?id="+data+"' class='btn btn-secondary btn-sm btn-icon icon-left'>修改</a>";
									html += "<a href='javascript:void(0);' onclick='deleteActivity(this);' id="+data+" class='btn btn-danger btn-sm btn-icon icon-left'>删除</a>";
									if(row.flag == 0){
									...
									html += "<a href='javascript:void(0);' onclick='topActivity(this);' id="+data+" class='btn btn-info btn-sm btn-icon icon-left'>置顶</a>";
								return html;
								}
							}
							],
        ...
        <section class="content-com">
            <header class="header-details">
                ...
                <div>
                    ...
                    <p><span><i id="price">$!{activity.price}</i>元/人</span></p>
                    <p><span><i id="beginTime">$!{beginTime}</i>&nbsp;&nbsp;<i id="endTime">$!{endTime}</i></span></p>
                    <p><span id="activityaddress">$!{activity.address}</span></p>  
                </div>
            </header>
