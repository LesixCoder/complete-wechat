...
<!-- 					    <a href="javascript:selectCourse();" class="btn btn-secondary btn-sm btn-icon icon-left" style="color:#ffffff;">查询</a> -->
					    <a href="goMatchCategorySave" class="btn btn-info btn-sm btn-icon icon-left" style="color:#ffffff;">添加</a>
					</h3>
				...
				<script type="text/javascript">
					...
					var matchCategoryList;
					jQuery(document).ready(function($){
						    matchCategoryList = jQuery("#example-2").dataTable({
							sAjaxSource: "queryMatchCategoryList",
							bServerSide: true,
							...
							"fnRowCallback": function (nRow, aoData, iDisplayIndex) {
							    if(aoData.content != "" && aoData.content != "null" && aoData.content != null){
							       if (aoData.content.length > 10) {
					                jQuery('td:eq(2)', nRow).html( aoData.content.substr(0, 15)+"...");
					               }
							    }
								...
								if(aoData.unlockContent != "" && aoData.unlockContent != null && aoData.unlockContent != "null"){
								   if (aoData.unlockContent.length > 10) {
					                jQuery('td:eq(3)', nRow).html( aoData.unlockContent.substr(0, 10)+"...");
					               }
								}
...
// 								    );
							...
							aoColumns: [
								...
								{"mDataProp": "unlockContent","sClass": "thead-tr-th"},
								{"mDataProp": "score","sClass": "thead-tr-th"},
								{"mDataProp": "medalName","sClass": "thead-tr-th"}
							],
							columnDefs: [
							{
								"targets": [0],
								...
								"render": function(data, type, row,full) { //返回自定义内容
									var html = "";
									html += "<a href='goMatchCategorySave?id="+data+"' class='btn btn-secondary btn-sm btn-icon icon-left'>修改</a>";
							...
							if(confirm("确定要删除?")){
								jQuery.ajax({
								    ...
								    if(data.flag = "success"){
								        alert("删除成功!");
								        matchCategoryList.fnPageChange(parseInt($(".pagination").find("li[class='paginate_button active'] a").html())-1);
								        matchCategoryList.fnReloadAjax();
								    }else{
								        alert("删除失败!");
								       }
								    },
					...
					<table class="table table-bordered table-striped " id="example-2" data-pattern="priority-columns">
						<thead>
							<tr>
								...
								<th style="font-size: 13px;">类别名称</th>
								<th style="font-size: 13px;">类别介绍</th>
								<th style="font-size: 13px;">解锁条件描述</th>
								<th style="font-size: 13px;">解锁积分</th>
								<th style="font-size: 13px;">解锁所需勋章</th>
							</tr>
						</thead>
