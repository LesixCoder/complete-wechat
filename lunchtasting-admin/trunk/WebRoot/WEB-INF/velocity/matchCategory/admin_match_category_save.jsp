...
<body class="page-body">
			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					...
					<form role="form" method="post" class="form-horizontal validate" id="restaurant-form" target="ifa">
						<input type="hidden" id="id" value="$!{matchCategory.id}">
						<div class="form-group" name="check-div">
							<label class="col-sm-2 control-label" >分类名称</label>
							<div class="col-sm-10">
							<input type="text" class="form-control" id="name" name="name" value="$!{matchCategory.name}" data-validate="required" data-message-required="名称为必填项" placeholder="输入名称" aria-describedby="name-error" aria-invalid="true" maxlength="3000">
							</div>
						</div>
						...
						<div class="form-group" name="check-div">
							...
							<div class="col-sm-10">
							<input type="text" class="form-control" id="content" name="content" value="$!{matchCategory.content}" data-validate="required" data-message-required="介绍为必填项" placeholder="输入介绍" aria-describedby="content-error" aria-invalid="true" maxlength="3000">
							</div>
						</div>
						<div class="form-group-separator"></div>
						<div class="form-group" name="check-div">
							<label class="col-sm-2 control-label" >解锁条件描述</label>
							<div class="col-sm-10">
							<input type="text" class="form-control" id="unlockContent" name="unlockContent" value="$!{matchCategory.unlockContent}" data-validate="required" data-message-required="解锁条件描述为必填项" placeholder="输入解锁条件描述" aria-describedby="unlockContent-error" aria-invalid="true" maxlength="3000">
							</div>
						</div>
						<div class="form-group-separator"></div>
						<div class="form-group" name="check-div">
							<label class="col-sm-2 control-label" for="restaurant.restaurantName">解锁积分</label>
							<div class="col-sm-10">
								<input type="text" class="form-control" id="score" name="score" value="$!{matchCategory.score}" data-validate="required" data-message-required="解锁积分为必填项" placeholder="输入解锁积分" aria-required="true" aria-describedby="score-error" aria-invalid="true" maxlength="30" onkeyup='this.value=this.value.replace(/\D/gi,"")'>
							</div>
						</div>
						<div class="form-group-separator"></div>
						<div class="form-group" name="check-div">
							<label class="col-sm-2 control-label" for="activity.name">所需勋章</label>
							<div class="col-sm-10" lang="div1">
							<div class="col-sm-12">
    								<select class="form-control selectpicker bla bla bli" data-live-search="true" id="medal" data-validate="required" data-message-required="" aria-required="true" aria-describedby="seller-error" aria-invalid="true" style="display: block; visibility: visible; width: 795px; height: 35px; opacity: 100; position: absolute; top: 0px; left: 0px; cursor: pointer; z-index: 999999; margin: 0px; padding: 6px; -webkit-appearance: menulist-button;">
    								#foreach ($medal in $medal)
										<option value="$!{medal.id}" "#if($medal.id == $!{matchCategory.medalId}) selected="selected" #end">$!{medal.name}</option>
									#end	
									</select>  
							</div>
							</div>
						...
						<div class="form-group" name="check-div">
							...
							<div class="col-sm-10">
								<input type="text" class="form-control" id="sort" name="sort" value="$!{matchCategory.sort}" placeholder="默认99，如不排序，无需填写" aria-required="true" aria-describedby="name-error" aria-invalid="true" maxlength="30" onkeyup='this.value=this.value.replace(/\D/gi,"")'>
							</div>
						</div>
						<div class="form-group-separator"></div>
	...
	var murl = "";
	var baseUrl = "$!{baseUrl}";
	var imgUrl = "$!{matchCategory.imgUrl}";
	...
	K("#course-save").click(function(){
		...
		if(checknum == 0){
				K.ajax('saveMatchCategory', function(data) {
				        show_loading_bar(100);
	  					if(data.flag == "success"){
	  						alert("保存成功!");
	  						location = "goMatchCategoryList";
	  					}else{
	  						alert(data.msg);
	  						return false;
	  					}
