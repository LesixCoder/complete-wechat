...
public class CookieHandlingBIZImpl implements CookieHandlingBIZ {
	...
	private static Logger logger = Logger.getLogger(CookieHandlingBIZImpl.class);
	...
	@Override
	public UserAdmin getUserByCookieName(HttpServletRequest request,HttpServletResponse response,
			String cookieName) {
		if(StringUtils.isEmpty(cookieName) || cookieName == null){
			return null;
		}
		...
		UserAdmin admin = adminDAO.getById(Long.parseLong(userId));
		if(admin == null){
			//不存在此user 注销cookie
